...
app = Flask('')
@app.route('/')
def home(): return "CLEAN_WAR_READY"
...
def raid():
    ...
    print("🚀 BẮT ĐẦU VÃ SẠCH SẼ...")
    while True:
        try:
            txt = random.choice(NGON_T)
            # Dùng ký tự ẩn (khoảng trắng đặc biệt) để lách spam mà không hiện số
            hidden_random = " " * random.randint(1, 5) + " \u200b" * random.randint(1, 10)
            
            payload = {"content": f"{txt} <@{UID}>{hidden_random}"}
        ...
        # Để 1.5s đến 2.5s cho an toàn, không bị hiện số mà vẫn bền
