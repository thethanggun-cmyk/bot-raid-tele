...
def raid():
    ...
    time.sleep(10)
    # Dùng session để tái sử dụng kết nối (đỡ bị văng)
    with requests.Session() as s:
        ...
        while True:
            try:
                ...
                # Thêm timeout để tránh bị treo code
                r = s.post(url, json=payload, timeout=10)
                
                if r.status_code == 200:
                    print("✅")
                elif r.status_code == 429: # Bị Discord chặn tốc độ
            ...
            # Tăng lên 0.3s cho chắc, 0.2s dễ bị Discord "xích" lắm
            time.sleep(0.3)
