...
app = Flask('')
@app.route('/')
def home(): return "GATLING_ULTRA_SMILE_ON"
...
# --- KHO ĐẠN 50+ CÂU KÈM ĐUÔI CƯỜI KHẨY ---
SMILE = "=)))))) =))))))) = )))))) =)))))) =))))))) = )))))) =)))))) =))))))) = ))))))"
DAN_DUOC = [
...
async def fire(session, channel_id):
    ...
    while True:
        try:
            # Gộp Chữ to + Tag + Nội dung khịa + Cười khẩy + Mã số chống trùng
            ...
            async with session.post(
                ...
                if r.status == 429:
                    ...
                    # Để 0.05 để vừa nhanh vừa không bị Render kick do quá tải
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = []
        for ch_id in CH_IDS:
            for _ in range(5): # Chạy 5 luồng cho mỗi kênh
                tasks.append(fire(session, ch_id))
        await asyncio.gather(*tasks)

if __name__ == "__main__":
    threading.Thread(target=run_web, daemon=True).start()
    print("--- ĐANG KÍCH HOẠT CHẾ ĐỘ 'CƯỜI KHẨY' ---")
    try:
        asyncio.run(main())
    except:
        pass
