...
@app.route('/')
def home():
    return "BOT_RAID_PHANTOM_READY"

def run_web():
    port = int(os.environ.get("PORT", 10000))
    app.run(host='0.0.0.0', port=port)

# 2. CẤU HÌNH LẤY TỪ ENVIRONMENT (BẢO MẬT)
TOKEN = os.getenv("USER_TOKEN")
CH_ID = os.getenv("CHANNEL_ID")
TAG_ID = os.getenv("TAG_ID")

# 3. KHO ĐẠN 100+ CÂU (XOAY VÒNG KHÔNG LẶP)
DAN_DUOC = [
...
def raid():
    if not TOKEN or not CH_ID:
        print("LỖI: Chưa cấu hình USER_TOKEN hoặc CHANNEL_ID trong Environment!")
        return
    ...
    # Chuẩn bị Tag (Dùng ID từ Environment)
    mention = f"<@{TAG_ID}> " if TAG_ID else ""
    
    # Trình xào đạn chống lặp
    pool = DAN_DUOC.copy()
    random.shuffle(pool)
    ...
    while True:
        try:
            ...
            if r.status_code == 429:
                ...
                print(f"Bị giới hạn, nghỉ {wait}s...")
                time.sleep(wait)
